...
class Path {
    static compute(root: TreeNode | undefined, sum: number): Observable<number[]> {
        if (root) {
            return Observable.create(
                ...
                .filter((node: Node) => node[0].isLeaf() && this.sumToCurrentNode(node) === sum)
                .map((node: Node) => node[1]);
        }
        return Observable.empty();
    }

    private static emission(emitter: Observer<Node>, node: Node): void {
        emitter.next(node);
    ...
    private static sumToCurrentNode(node: Node): number {
